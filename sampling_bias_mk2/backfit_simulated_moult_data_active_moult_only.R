...
uz2r_iter = as.numeric(args[2])
  uz2r <- try(moultmcmc('pfmg','yday','ID',active_moult_recaps_only = TRUE ,data = sim_data[[i]], flat_prior = FALSE, chains = 2, log_lik = FALSE, cores = 2, warmup = min(floor(uz2r_iter/2),2000), iter = uz2r_iter,thin=floor(uz2r_iter/2000)))
  saveRDS(uz2r,paste0('simulation_outputs/biased_sampling/uz2ra_scenario',formatC(i,flag=0,width=3),'.rds'),compress = 'bzip2')
#}
